...
do
    ...
    if (F_A) then
        ...
        return Notify(nil, "Loaded", "drills admin is already loaded... use 'killscript' to kill", nil),
...
AddCommand("fling3", {"f3"}, "fling3", {}, function(Caller)
...
AddCommand("getmass", {"gm"}, "Get players mass.", {}, function(Caller)
...
AddCommand("reweld", {"rw"}, "enlarges your item", {}, function(Caller)
...
AddCommand("advertise", {}, "advertises the script", {}, function()
    local ChatRemote = Services.ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest
    ChatRemote.FireServer(ChatRemote, "drills priv admin v2", "All");
end)
...
AddCommand("joinserver", {"discord"}, "joins the fates admin discord server", {}, function()
    ...
    if (Request({
        ...
        return "joined drills discord server"
    else
...
AddCommand("changelogs", {"cl"}, "shows you the updates on drills admin", {}, function()
...
Utils.Notify(LocalPlayer, "Welcome to drills private admin", "'cmds' to see all of the commands, 'config' to customise the script");
